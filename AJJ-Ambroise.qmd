# Aline Julienne Joséphine Ambroise (1837-1865) {#ajj}
...
```{r carte-france-villes, echo=FALSE, fig.cap="Carte de France avec Doullens, Amiens et Paris", fig.width=6, fig.height=6}
library(ggplot2)
library(maps)

# Données des villes
villes <- data.frame(
  nom = c("Doullens", "Amiens", "Paris"),
  lon = c(2.34, 2.3, 2.35),
  lat = c(50.15, 49.9, 48.85)
)

# Fond de carte France
france <- map_data("france")

# Carte avec remplissage transparent (alpha = 0.2)
ggplot() +
  geom_polygon(
    data = france,
  ...
  geom_text(data = villes, aes(x = lon, y = lat, label = nom), vjust = -1) +
  coord_fixed(1.3) +
  theme_minimal()
```
...
## Les métiers de Julienne et ses enfants
...
#| echo: FALSE
#| fig-align: center
#| fig-cap: "Paris - La rue de Passy dans le XVI° arrondissement de Paris : Avant 1860"
knitr::include_graphics("Images/1860-paris-rue-passy.jpg")
```
...
Pour Julienne, née en 1837 et vivant entre Doullens, Amiens et Paris, ce fut un passage du monde artisanal provincial à la vie urbaine. Elle vit défiler les régimes, les gares et les ateliers («piqueuse de bottines», lingère), entre la naissance de sept enfants et une disparition prématurée en 1865. Son parcours reflète à son échelle les bouleversements de la France: rurale, puis industrielle ; provinciale, puis urbaine. Pour saisir l'univers dans lequel a évolué Julienne, deux œuvres offrent des éclairages complémentaires remarquables. Le film de Cédric Klapisch La venue de l'avenir [@klapisch2025venue] restitue avec justesse l'ambiance de cette époque charnière du XIXe siècle et nourrit l'imaginaire que nous pouvons nous faire de cette femme en transit entre la France rurale et urbaine. Mais c'est surtout L'Assommoir de Zola qui demeure la référence absolue pour comprendre l'univers quotidien de Julienne : une femme seule, mère de plusieurs enfants, exerçant des métiers précaires (lingère, piqueuse de bottines), dans le Paris ouvrier du Second Empire. Le parallèle entre Gervaise Macquart et notre ancêtre est saisissant, jusque dans les détails sociologiques et géographiques - même précarité, mêmes métiers de l'aiguille et du linge, même solitude face à la maternité, même errance entre les quartiers populaires de la capitale. Ces deux œuvres, l'une contemporaine, l'autre historique, permettent de donner chair et relief à ce que les registres d'état civil ne peuvent restituer : la texture d'une existence. 
